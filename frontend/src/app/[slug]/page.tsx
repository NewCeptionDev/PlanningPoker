...
export default function Page({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    async function fetchLobbyInformation() {
      const res = await fetch(baseUrl + '/api/management/lobbyInformation/' + params.slug, {
        headers: {
    ...
    return (
      <>
        <Headline
          ...
          leaveAction={() => router.push('/')}
          userName={undefined}
        />
        <div
          className='flex flex-col items-center h-[91vh] justify-center'
          ...
          <form
            action={(formData: FormData) =>
              setUser({
                ...
                roles: [roleFromString(formData.get('role')?.toString() || '')],
              })
            }
